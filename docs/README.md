# Documentation

## Getting Started

| Document | Description |
|----------|-------------|
| [README.md](../README.md) | Project overview, install, quick start |
| [docs/hermes-agent-guide.md](hermes-agent-guide.md) | Hermes Agent operators guide |
| [docs/examples/first-run.md](examples/first-run.md) | First-run walkthrough |
| [docs/development.md](development.md) | Development environment |

## Using Hardproof

| Document | Description |
|----------|-------------|
| [docs/profiles.md](profiles.md) | Quick, Standard, and Critical run profiles |
| [docs/protocol.md](protocol.md) | Protocol specification |
| [docs/command-reference.md](command-reference.md) | All commands, tools, and surfaces |
| [docs/configuration-and-migrations.md](configuration-and-migrations.md) | Configuration schema and migrations |
| [docs/policy-packs.md](policy-packs.md) | Language policy packs (Python, Node, Rust, Go) |

## Architecture

| Document | Description |
|----------|-------------|
| [docs/architecture.md](architecture.md) | System architecture |
| [docs/security-model.md](security-model.md) | Security model |
| [docs/compatibility.md](compatibility.md) | Compatibility and versioning |
| [docs/repository-map.md](repository-map.md) | Repository layout |

## Policy and Waivers

| Document | Description |
|----------|-------------|
| [docs/configuration-and-migrations.md](configuration-and-migrations.md) | Policy configuration and migration |
| [docs/policy-packs.md](policy-packs.md) | Built-in language policy packs |
| [docs/specs/v0.2.0-gatehouse.md](../docs/specs/v0.2.0-gatehouse.md) | Gatehouse specification |
| [docs/specs/v0.3.0-workcells.md](specs/v0.3.0-workcells.md) | Workcells specification |
| [docs/research/hermes-child-session-api-v0.3.0.md](research/hermes-child-session-api-v0.3.0.md) | Public Hermes child-session compatibility research |

## Release Evidence

| Document | Description |
|----------|-------------|
| [CHANGELOG.md](../CHANGELOG.md) | Release changelog |
| [ROADMAP.md](../ROADMAP.md) | Roadmap |
| [STATUS.md](../STATUS.md) | Current release status |
| [docs/releasing.md](releasing.md) | Release process |
| [docs/release/hardproof-v0.2.0-independent-audit.md](release/hardproof-v0.2.0-independent-audit.md) | v0.2.0 independent audit |
| [docs/release/hardproof-v0.2.0-security-review.md](release/hardproof-v0.2.0-security-review.md) | v0.2.0 security review |
| [docs/release/hardproof-v0.2.0-compatibility.md](release/hardproof-v0.2.0-compatibility.md) | v0.2.0 compatibility evidence |
| [docs/release/hardproof-v0.2.0-installation-test.md](release/hardproof-v0.2.0-installation-test.md) | v0.2.0 installation test |

## Security

| Document | Description |
|----------|-------------|
| [SECURITY.md](../SECURITY.md) | Security policy and vulnerability reporting |
| [docs/security/release-signing.md](security/release-signing.md) | Release signing policy |

## Contributing

| Document | Description |
|----------|-------------|
| [CONTRIBUTING.md](../CONTRIBUTING.md) | Contribution guidelines |
| [AGENTS.md](../AGENTS.md) | Coding agent instructions |
| [GOVERNANCE.md](../GOVERNANCE.md) | Project governance |
| [CODE_OF_CONDUCT.md](../CODE_OF_CONDUCT.md) | Code of conduct |
| [SUPPORT.md](../SUPPORT.md) | Support |

## ADRs

| Document | Description |
|----------|-------------|
| [docs/adr/0001-clean-room-implementation.md](adr/0001-clean-room-implementation.md) | Clean-room implementation |
| [docs/adr/0002-standalone-hermes-plugin.md](adr/0002-standalone-hermes-plugin.md) | Standalone Hermes plugin |
| [docs/adr/0003-project-local-sqlite.md](adr/0003-project-local-sqlite.md) | Project-local SQLite |
| [docs/adr/0004-evidence-freshness.md](adr/0004-evidence-freshness.md) | Evidence freshness |
| [docs/adr/0005-stage-aware-tool-policy.md](adr/0005-stage-aware-tool-policy.md) | Stage-aware tool policy |
| [docs/adr/0006-ordered-policy-traces.md](adr/0006-ordered-policy-traces.md) | Ordered policy traces |
| [docs/adr/0007-public-rebrand-hardproof.md](adr/0007-public-rebrand-hardproof.md) | Public rebrand to Hardproof |
| [docs/adr/0008-workcell-task-attempt-state-machines.md](adr/0008-workcell-task-attempt-state-machines.md) | Workcell state machines |
| [docs/adr/0009-shared-workspace-workcell-scheduling.md](adr/0009-shared-workspace-workcell-scheduling.md) | Shared-workspace scheduling |
| [docs/adr/0010-workcell-child-result-validation.md](adr/0010-workcell-child-result-validation.md) | Child-result validation |
| [docs/adr/0011-provider-neutral-workcell-model-tiers.md](adr/0011-provider-neutral-workcell-model-tiers.md) | Provider-neutral model tiers |
| [docs/adr/0012-transactional-workcell-claims.md](adr/0012-transactional-workcell-claims.md) | Transactional claims |
| [docs/adr/0013-workcell-interruption-recovery.md](adr/0013-workcell-interruption-recovery.md) | Interruption recovery |

## Historical Documents

| Document | Description |
|----------|-------------|
| [docs/rebrand/hardproof-rename-manifest.md](rebrand/hardproof-rename-manifest.md) | Crucible to Hardproof rename manifest |
| [docs/rebrand/hardproof-residual-name-audit.md](rebrand/hardproof-residual-name-audit.md) | Residual name audit |
| [docs/plans/v0.2.0-implementation.md](plans/v0.2.0-implementation.md) | v0.2.0 implementation plan (historical) |
| [docs/plans/v0.2.0-remaining-work.md](plans/v0.2.0-remaining-work.md) | v0.2.0 remaining work (historical at completion) |
