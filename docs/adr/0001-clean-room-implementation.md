...
Hardproof coordinates a staged software-engineering process for Hermes Agent. Other agent workflow projects explore related ideas, so the repository needs a durable originality boundary. The working name also overlaps with existing commercial uses and has not completed trademark or marketplace clearance.
...
Hardproof will be designed and implemented as a clean-room project from its own specification and current, documented public Hermes Agent APIs.
...
"Hardproof" remains a working name until trademark and marketplace clearance is complete. Public materials will use the distinct identifiers `hardproof`, `hardproof`, and `hardproof`, will avoid Atlassian trade dress and terminology, and will state that no Atlassian affiliation is implied.
...
- Every feature and document must have an independently traceable origin in Hardproof requirements or public Hermes contracts.
