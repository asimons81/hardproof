...
from hardproof.services.evidence import EvidenceService
...
class VerificationHook:
    ...
    def __call__(self, **kwargs: Any) -> dict[str, str] | None:
        ...
        return {
            "action": "continue",
            "message": (
                f"Hardproof evidence is missing, failed, indeterminate, or stale: {blocker}. "
                f"Current stage: {run.stage.value}. Invoke hardproof_verify before completion."
            ),
        }
