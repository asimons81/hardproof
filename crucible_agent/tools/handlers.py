...
from crucible_agent.domain.enums import ArtifactKind, RiskLevel, RunStage, TaskStatus
...
def _error(exc: Exception) -> dict[str, Any]:
    logger.info("Crucible tool request failed: %s", type(exc).__name__)
...
def create_handlers(dependencies: HandlerDependencies) -> dict[str, Callable[..., str]]:
    ...
    def run(args: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == "start":
            ...
            raise ValueError(f"unknown crucible_run action: {action}")
    ...
    def record(args: dict[str, Any]) -> dict[str, Any]:
        ...
        if kind in {"approval", "waiver"}:
            return {"ok": False, "error": "Human approvals and waivers require /crucible approve or /crucible waive."}
        ...
        if kind not in kind_map:
            if "approv" in kind.lower() or "waiv" in kind.lower():
                return {"ok": False, "error": "Human approvals and waivers require /crucible approve or /crucible waive."}
            raise ValueError(f"unknown crucible_record kind: {kind}")
    ...
    def task(args: dict[str, Any]) -> dict[str, Any]:
        ...
        raise ValueError(f"unknown crucible_task action: {action}")
    ...
    return {
        "crucible_run": wrap(run),
        "crucible_record": wrap(record),
        "crucible_task": wrap(task),
        "crucible_transition": wrap(transition),
        "crucible_verify": wrap(dependencies.verify),
        "crucible_report": wrap(dependencies.report),
    }
