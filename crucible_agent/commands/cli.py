"""`hermes crucible` argparse adapter."""
...
def _configure(parser: argparse.ArgumentParser) -> None:
    sub = parser.add_subparsers(dest="crucible_command", required=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="hermes crucible")
    _configure(parser)
    return parser


def _to_argv(args: argparse.Namespace) -> list[str]:
    command = args.crucible_command
    if command == "start":
...
def register_cli(ctx: Any, service_factory: Callable[[], CommandService]) -> None:
    ...
    ctx.register_cli_command(
        "crucible",
        "Manage persistent Crucible engineering runs",
        _configure,
        handler,
        description="Start, inspect, approve, pause, verify, and export Crucible runs",
    )
