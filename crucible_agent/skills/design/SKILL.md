---
name: design
description: Shape the smallest reversible Crucible design from discovery evidence during DESIGN, including contracts, failures, and risks.
---
...
4. Record consequential choices with `crucible_record` as decisions.
5. Write the design artifact with `crucible_record`.
...
## Verification
Cross-check every interface claim and unresolved risk, then ask the human to use `/crucible approve design`. After approval exists, call `crucible_transition` for PLAN.
