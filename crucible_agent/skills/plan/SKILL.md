---
name: plan
description: Turn an approved Crucible design into dependency-aware, independently testable tasks during PLAN without editing product code.
---
...
4. Add each task through `crucible_task`.
5. Record the complete plan through `crucible_record`.
...
## Verification
Check task coverage and dependency order, then ask the human to use `/crucible approve plan`. After approval exists, call `crucible_transition` for IMPLEMENT.
