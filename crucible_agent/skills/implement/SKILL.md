---
name: implement
description: Execute approved Crucible tasks during IMPLEMENT with focused tests, durable task updates, and scope-controlled code changes.
---
...
## Procedure
1. Select a dependency-ready task with `crucible_task`.
...
5. Update the task with acceptance notes through `crucible_task`.
6. Record consequential deviations with `crucible_record`; return to DESIGN or PLAN when approval assumptions no longer hold.
...
## Verification
Run task-focused checks, review the actual diff, and call `crucible_transition` for REVIEW. Full verification evidence belongs to VERIFY.
